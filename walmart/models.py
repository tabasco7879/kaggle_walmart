...
def eval_model(train, valid, Y_hat, column=None):
    ...
    if column is not None:
        t_error = fun_log_error(Y_hat[:ntrain], \
                                train[column].values[:,np.newaxis], ntrain, 1, 0)
        v_error = fun_log_error(Y_hat[ntrain:ntrain + nvalid], \
                                valid[column].values[:,np.newaxis], nvalid, 1, 0)
    else:
        t_error = fun_log_error(Y_hat[:ntrain], \
...
def build_model3(train, valid, test, \
    ...
    Y = np.zeros((n, m))
    if column is not None:
        Y[0:ntrain] = train[column][:,np.newaxis]
    else:
        Y[0:ntrain] = train.values
    ...
    while (True): # Y_hat is a flatten format
        ...
        e1, e2 = eval_model(train, valid, Y_hat, column)
        Y_hat=Y_hat.flatten()
...
def build_model5(train, valid, test, \
                 ...
                 valid_init=None, \
                 column=None,\
                 alpha_train=1, alpha_unknown=0.001,
                 eps=1e-5, max_iter=10):
    # count the total number of rows
    ntrain, m = train.values.shape
    if column is not None:
    ...
    fmat_weight[fmat==0]=0
    fmat_weight=fmat_weight.flatten()
    ...
    while (True): # Y_hat is a flatten format
        ...
        D=compute_D(Y_hat, Y_shape)
        # run optimiaztion of hidden features
        fmat_weight, fval, _  = fmin_l_bfgs_b(cost_fun5, fmat_weight, g_cost_fun52, \
